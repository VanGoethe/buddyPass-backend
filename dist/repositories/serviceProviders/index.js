...
exports.PrismaServiceProviderRepository = void 0;
exports.createServiceProviderRepository = createServiceProviderRepository;
const serviceProviders_1 = require("../../models/serviceProviders");
class PrismaServiceProviderRepository {
    constructor(prisma) {
        this.prisma = prisma;
    }
    async create(data) {
        try {
            ...
            return serviceProviders_1.ServiceProvider.fromPrisma(result);
        }
        catch (error) {
            if (error.code === "P2002") {
    ...
    async findById(id, includeCountries = false) {
        try {
            const include = {};
            if (includeCountries) {
            ...
            return serviceProvider
                ? serviceProviders_1.ServiceProvider.fromPrisma(serviceProvider)
                : null;
        }
        catch (error) {
    ...
    async findMany(options = {}) {
        try {
            const { page = 1, limit = 10, sortBy = "createdAt", sortOrder = "desc", search, countryId, } = options;
            ...
            return {
                serviceProviders: serviceProviders.map((sp) => serviceProviders_1.ServiceProvider.fromPrisma(sp)),
                total,
            };
        }
        catch (error) {
    ...
    async delete(id) {
        try {
            ...
            if (error.code === "P2003") {
                throw new Error("Cannot delete service provider: it has associated subscriptions");
            }
    ...
    async existsByName(name, excludeId) {
        try {
            ...
            throw new Error(`Failed to check service provider name: ${error.message}`);
        }
    }
    async addSupportedCountries(serviceProviderId, countryIds) {
        try {
    ...
    async removeSupportedCountries(serviceProviderId, countryIds) {
        try {
    ...
    async getSupportedCountries(serviceProviderId) {
        try {
...
exports.PrismaServiceProviderRepository = PrismaServiceProviderRepository;
/**
 * Factory function to create ServiceProvider repository
 */
function createServiceProviderRepository(prisma) {
