...
export class SubscriptionService implements ISubscriptionService {
  ...
  async createSubscription(
    ...
    if (data.countryId) {
      await this.validateCountryForServiceProvider(
        data.countryId,
        data.serviceProviderId
      );
    }
    ...
    const emailExists = await this.subscriptionRepository.existsByEmail(
      data.email
    );
    if (emailExists) {
    ...
    const subscriptionWithCountry = await this.subscriptionRepository.findById(
      subscription.id,
      true
    );

    return new Subscription(subscriptionWithCountry!).toResponse();
  }
   ...
   */
  private async validateCountryForServiceProvider(
    countryId: string,
    serviceProviderId: string
  ): Promise<void> {
    ...
    if (!serviceProvider || !serviceProvider.supportedCountries) {
      throw new Error(
        "Service provider not found or has no supported countries"
      );
    }

    const supportedCountryIds = serviceProvider.supportedCountries.map(
      (rel) => rel.country.id
    );

    if (!supportedCountryIds.includes(countryId)) {
      throw new Error(
        `Country ${country.name} is not supported by this service provider`
      );
    }
  }
}
